...
class PadmaSociableBlockOptions extends PadmaBlockOptionsAPI {
	...
	public static function get_author_data($icon_set ){
		...
		if ( $icon_set != 'custom' ) {
			...
			$path 		= plugin_dir_path( __FILE__ ) . 'icons/' . $icon_set . '/author.txt';
	...
	public static function get_icons( $icon_set ) {
		...
		if ( $icon_set != 'custom' ) {
			...
			$path 		= plugin_dir_path( __FILE__ ) . 'icons/' . $icon_set . '/';
			$results 	= scandir($path);
			$icons 		= array();
